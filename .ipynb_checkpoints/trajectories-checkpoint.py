...
class Trajectories:
    ...
    def __getitem__(self, item):
        ...
        observations = torch.zeros(
            [length, self.num_nodes], device=self.device)
        row = torch.arange(length).unsqueeze(
            1).repeat(1, self._weights.shape[1])
    ...
    def leg_shortest_lengths(self, trajectory_id):
        ...
        min_distances = torch.zeros(
            num_jumps, device=self.device, dtype=torch.long)
    ...
    def write_to_directory(self, directory):
        ...
        if self._mask is not None:
            logging.warning(
                "Trajectories mask ignored when writing to directory")
    ...
    def read_from_files(
        ...
        if paths_filename is not None and os.path.exists(paths_filename):
            with open(paths_filename) as f:
                ...
                paths = torch.zeros(
                    [num_paths, max_path_length], dtype=torch.long) - 1
                for i, line in enumerate(f.readlines()):
