...
class Metric:
    ...
    def __init__(self, n_node):
        self.sum = 0.0
        self.count = 0
        self.vec_sum = torch.zeros(n_node)
    ...
    def vector_add(self, v):
        self.vec_sum = self.vec_sum.to(v.device)+v

    def vector_mean(self):
        self.vec_sum/self.count
...
class Evaluator:
    ...
    def init_metrics(self):
        ...
        for m in metrics:
            self.metrics[m] = Metric(self.n_node)
        for m in new_metrics:
            self.new_metrics[m] = Metric(self.n_node)
    ...
    def compute(
        ...
        with torch.no_grad():
            for trajectory_idx in range(len(trajectories)):
                ...
                # node_accuracy = [ for p in predictions]
    ...
    def get_metrics(self):
        ...
        for m, v in self.metrics.items():
            metrics[m] = v.mean()
        for m, v in self.new_metrics.items():
            new_metrics[m] = v.vector_mean()
    ...
    def update_metrics(
        ...
        top5_contains_target = compute_topk_contains_target(
            target_distributions, predictions, k=2)
        self.metrics["precision_top2"].add_all(top5_contains_target)
        top5_contains_target = compute_topk_contains_target(
            target_distributions, predictions, k=3)
        self.metrics["precision_top3"].add_all(top5_contains_target)
        top5_contains_target = compute_topk_contains_target(
            target_distributions, predictions, k=5)
        self.metrics["precision_top5"].add_all(top5_contains_target)

        # nodes where the prediction is right in top1 or top5
        node_to_acc1 = torch.index_select(target_distributions, 0, torch.nonzero(
            top1_contains_target).ravel()).sum(dim=0)
        node_from_acc1 = torch.index_select(observations[starts], 0, torch.nonzero(
            top1_contains_target).ravel()).sum(dim=0)
        node_to_acc5 = torch.index_select(target_distributions, 0, torch.nonzero(
            top5_contains_target).ravel()).sum(dim=0)
        node_from_acc5 = torch.index_select(observations[starts], 0, torch.nonzero(
            top5_contains_target).ravel()).sum(dim=0)

        #print("node to acc size:", node_to_acc1.shape)
        #print("node to acc size:", node_from_acc1.shape)
        #print("node to acc size:", target_distributions.shape)
        # print("node to acc size:", .shape)
        self.new_metrics["node_to_acc1"].vector_add(node_to_acc1)
        self.new_metrics["node_to_acc5"].vector_add(node_to_acc5)
        self.new_metrics["node_from_acc1"].vector_add(node_from_acc1)
        self.new_metrics["node_from_acc5"].vector_add(node_from_acc5)
        self.new_metrics["node_to_count"].vector_add(
            target_distributions.sum(dim=0))
        self.new_metrics["node_from_count"].vector_add(
            observations[starts].sum(dim=0))
